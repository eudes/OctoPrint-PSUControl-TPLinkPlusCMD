...
__author__ = "Eudes Ochoa <eudes8@gmail.com>"
...
class PSUControl_TPLinkPlusCMD(octoprint.plugin.StartupPlugin,
    ...
    def get_update_information(self):
        return dict(
            psucontrol_tplink=dict(
                displayName="PSU Control - TPLink Plus CMD",
                displayVersion=self._plugin_version,

                # version check: github repository
                type="github_release",
                user="eudes",
                repo="OctoPrint-PSUControl-TPLinkPlusCMD",
                current=self._plugin_version,

                # update method: pip w/ dependency links
                pip="https://github.com/eudes/OctoPrint-PSUControl-TPLinkPlusCMD/archive/{target_version}.zip"
            )
        )

    def run_cmd_for_state(self, state):
        if state == 1:
            self.run_command(self.config['oncommand'])
        else:
            self.run_command(self.config['offcommand'])
    ...
    def run_command(self, cmd):
        ...
        try:
            ...
            self._logger.debug("stdout: {}".format(result.stdout))
            self._logger.debug("stderr: {}".format(result.stderr))
            raise

__plugin_name__ = "PSU Control - TPLink Plus CMD"
__plugin_pythoncompat__ = ">=3.4,<4"

def __plugin_load__():
    global __plugin_implementation__
    __plugin_implementation__ = PSUControl_TPLinkPlusCMD()
